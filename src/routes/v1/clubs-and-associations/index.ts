...
const clubsInfo: FastifyPluginAsync = async (
  ...
  async function uploadImage(filePath: string, fileName: string, db: Db) {
  ...
  async function uploadClubs(data: ClubInfo[], db: Db) {
    ...
    for (const club of data) {
      try {
        ...
        throw new Error(
          `Logo path for club ${club.name} is invalid. Current path: ${BASE_IMAGE_PATH}${club.imageName}`,
        );
      }
    }
    for (const club of data) {
      await uploadImage(
        `${BASE_IMAGE_PATH}${club.imageName}`,
        club.imageName,
        db,
      );
  ...
  async function deleteClubs(names: string[], db: Db) {
    const collection = db.collection("clubs");
    const bucket = new GridFSBucket(db, { bucketName: "images" });
    ...
    "/clubs",
    {
      schema: {
        summary:
          "Get all clubs, optionally filtered by a search string of name.",
        description:
          "If no search string is specified, all clubs will be returned.",
        tags: ["Clubs"],
  ...
  fastify.post(
    "/uploadClubs",
    {
      schema: {
        ...
        description:
          "Upload or update club information. If a club with the same name exists, it will be updated.",
        tags: ["Clubs"],
  ...
  fastify.post(
    "/deleteClubs",
    {
      schema: {
        ...
        description:
          "Delete clubs by providing an array of club names. This will also delete associated images from GridFS.",
        tags: ["Clubs"],
        body: Type.Object({
