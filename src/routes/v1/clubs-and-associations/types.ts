...
const allowedSocialMediaTypes = Object.keys(SocialMediaType).filter(k => isNaN(Number(k)));

export const SocialMediaTypeSchema = Type.Union(
  allowedSocialMediaTypes.map(type => Type.Literal(type))
);

const allowedClubCategories = Object.keys(ClubCategory).filter(k => isNaN(Number(k)));
...
export const SocialMediaInfoSchema = Type.Object({
  ...
  link: Type.String({
    ...
    examples: ['https://forms.gle/2AuUSNX3jkzHf8Z99', "https://www.linkedin.com/company/the-business-students'%E2%80%8B-union-hkustsu/"]
  }),
})
...
export const ClubInfoWithImageSchema = Type.Intersect([
  ...
  Type.Object({ 
    imageBase64: Type.String({
      ...
      examples: ['iVBORw0KGgoAAAANSUhEUgAAAOEAAADhCAYAAAA+5n6JAAAABmJLR0QA/wD/AP+gvaeTAAAX...'],
    })
  })
]);
